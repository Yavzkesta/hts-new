...
export default ({ config: _themeConfig = 'src/config.yaml' } = {}): AstroIntegration => {
  let cfg: AstroConfig;
  return {
    name: 'HebergTonServ-integration',
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('HebergTonServ');

        const virtualModuleId = 'HebergTonServ:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-HebergTonServ-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`HebergTonServ \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`HebergTonServ config has been loaded.`);
        }
      },
